document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateFigureDescription(selectedFigure) {
        ...
        figureDescription.innerHTML = `
            ...
            <p class="text-[36px]">${figureData.name}</p>
            <p class="text-[16px]">${figureData.bio}</p>
            <p class="text-[26px] mt-[32px]">测试小标题</p>
            <p class="text-[16px]">${figureData.age}</p>
    ...
    for (const figureKey in figures) {
        if (figures.hasOwnProperty(figureKey)) {
            const button = document.createElement('button');
            button.textContent = figures[figureKey].name;
            button.classList.add('text-left', 'w-full', 'px-6', 'hover:bg-[#F3F4F6]', 'font-medium', 'py-4');
            button.dataset.figure = figureKey; // 使用data属性存储人物标识

            button.addEventListener('click', function () {
                updateFigureDescription(figureKey);
            });
